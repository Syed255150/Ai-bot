...
def remove_product(group_id):
    def fetch_products_in_group(group_id):
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute('''
        SELECT product.product_id, product.product_name FROM product
        INNER JOIN group_products ON product.product_id = group_products.product_id
...
def remove_products_GUI(group_id):
    remove_product(group_id)