...
# *************************
    ...
    def delete_selected_group():
        selected_idx = group_listbox.curselection()
        print("in the delete group : ", group_ids)
        if not selected_idx:
            messagebox.showwarning("No Selection", "Please select a group.")
            return
...
def register():
    def create_account():
        ...
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO users (username, password, machine_address) VALUES (?, ?, ?)", (username, password, mac_address))
        cursor.execute("INSERT INTO access (username, batch_size) VALUES (?, '1')", (username,))
        conn.commit()
        conn.close()

        messagebox.showinfo("Registration Successful", "Your account has been created. Please wait for admin validation.")
...
#     open_cmd_and_run_script('scheduler.py')
